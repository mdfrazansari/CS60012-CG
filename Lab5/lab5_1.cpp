...
int main() {
	...
	// delete common/duplicate vertices and also update their references in faces
	cerr << "Removing duplicate vertices..." << endl; 
...
{
	// sort vertices using deleted field
	sort(vertices.begin(), vertices.end(), sortDeletePoint);
	int index = 1;
	// reindex vertices
...
void Obj::removeDuplicateVertices()
{
	...
	// sort vertices in lex order (x then y then z)
	sort(v2.begin(), v2.end(), comparePoint);
	vector<int> toRemove(vertices.size());
	int count = 0;	
	// find duplicate vertices to be removed
	...
	// update reference of 'to be deleted' vertices in faces to point to other equivalent vertices
	...
	// either remove vertices permanently or set deleted flag to true
	int index = 1;
...
int sortDeleteFace(const Face *f1, const Face *f2)
{
	return f2->deleted > f1->deleted;
}


int sortDeletePoint(const Point *p1, const Point *p2)
{
	return p2->deleted > p1->deleted;
}
